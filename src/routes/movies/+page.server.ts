...
export const load: PageServerLoad = async ({ url, locals }) => {
	...
	// Library movies from media servers (Jellyfin, etc.)
	const { items: libraryItems, total } = await getLibraryItems(
		{ type: 'movie', sortBy, limit: 200 },
		userId
	);

	// Overseerr popular + trending movies (cached 2 min)
	...
	let popularMovies: UnifiedMedia[] = [];
	let trendingMovies: UnifiedMedia[] = [];

	if (adapter?.discover) {
		[popularMovies, trendingMovies] = await Promise.all([
			withCache('movies:popular', 120_000, async () => {
			...
			withCache('movies:trending', 120_000, async () => {
				const items: UnifiedMedia[] = [];
				await Promise.allSettled(
					overseerrConfigs.map(async (c) => {
						const cred = userId ? getUserCredentialForService(userId, c.id) ?? undefined : undefined;
	...
	return { libraryItems, total, sortBy, popularMovies, trendingMovies, hasOverseerr };
};
