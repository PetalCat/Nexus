...
<div class="flex flex-col gap-10 pb-10">
	...
	{#await data.trendingMovies}
		...
		{/if}
	{:then trendingMovies}
		{@const hero = pickHero(trendingMovies)}
		{#if hero}
			<HeroSection
				mode="browse"
			...
			>
				<div class="flex h-full items-end p-4 pb-5 sm:p-6 sm:pb-8 md:p-8 md:pb-10">
					...
					<span class="ml-auto hidden select-none text-[clamp(2rem,6vw,5rem)] font-black tracking-tight text-cream/[0.03] sm:block">MOVIES</span>
				</div>
			</HeroSection>
		{/if}
	{:catch}
