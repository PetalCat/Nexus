				...
				<div class="absolute bottom-0 left-0 right-0 flex items-end p-4 pb-5 sm:p-6 sm:pb-8 md:p-8 md:pb-10">
					<div class="max-w-xl">
						<div class="mb-3 flex flex-wrap items-center gap-2">
							...
							{#if h.duration}<span class="text-xs text-cream/30">·</span><span class="text-xs font-medium text-cream/50">{formatDuration(h.duration)}</span>{/if}
							{#if h.rating}
						...
						<div class="mt-4 flex items-center gap-3">
							<a href="/media/{h.type}/{h.sourceId}?service={h.serviceId}" class="flex items-center gap-2 rounded-xl border border-cream/20 bg-cream/10 px-5 py-2.5 text-sm font-semibold text-cream backdrop-blur-sm transition-all hover:bg-cream/20 active:scale-95">More Info</a>
						</div>
					</div>
				</div>
			</div>
