<script lang="ts">
	...
	// Pick hero from trending TV: prefer item with backdrop + description
	const hero = $derived.by<UnifiedMedia | null>(() => {
		const all = data.trendingTV;
		return (
			all.find((i) => i.backdrop && i.description) ??
			all.find((i) => i.backdrop) ??
			null
		);
	});

	const popularRow = $derived.by<DashboardRow>(() => ({
		id: 'popular-tv',
...
<div class="flex flex-col gap-10 pb-10">
		...
		>
			...
			{:else}
				<div
					...
					style="background: linear-gradient(135deg, #1a1a2e 0%, #0d0d18 60%, #12122a 100%)"
				>
					<div
						class="absolute inset-0 opacity-20"
						style="background: radial-gradient(ellipse at 30% 50%, #7c6cf8 0%, transparent 60%)"
					></div>
				</div>
			{/if}
			...
			<div
				class="absolute inset-0"
				style="background: linear-gradient(to top, #05050a 0%, rgba(5,5,10,0.65) 35%, rgba(5,5,10,0.1) 70%, transparent 100%)"
			></div>
			<div
				class="absolute inset-0"
				style="background: linear-gradient(to right, rgba(5,5,10,0.85) 0%, rgba(5,5,10,0.4) 40%, transparent 70%)"
			></div>
			...
			>
				<div class="max-w-xl">
					<div class="mb-3 flex flex-wrap items-center gap-2">
						...
						{#if h.year}<span class="text-xs font-medium text-white/50">{h.year}</span
							>{/if}
						{#if h.duration}
							<span class="text-xs text-white/30">·</span>
							<span class="text-xs font-medium text-white/50"
								>{formatDuration(h.duration)}</span
							>
						{/if}
						{#if h.rating}
							<span class="text-xs text-white/30">·</span>
							<span
								class="flex items-center gap-0.5 text-xs font-medium text-[var(--color-star)]"
							>
					...
					<h1
						class="text-display text-2xl font-bold leading-tight drop-shadow-2xl sm:text-3xl md:text-4xl"
					>
						{h.title}
					</h1>
					...
					{#if h.genres?.length}
						<div class="mt-2 flex flex-wrap gap-1.5">
							{#each h.genres.slice(0, 3) as genre}
								<span
									class="rounded-full border border-white/20 px-2 py-0.5 text-[10px] font-medium text-white/70"
									>{genre}</span
								>
							{/each}
						</div>
					...
					{#if h.description}
						<p
							class="mt-3 line-clamp-2 max-w-lg text-sm leading-relaxed text-white/70 sm:line-clamp-3"
						>
							{h.description}
						</p>
					{/if}
	...
	<div class="px-3 sm:px-4 lg:px-6">
		<div class="mb-4 flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center">
			<div>
				...
				<p class="mt-0.5 text-xs text-[var(--color-subtle)]">
	...
	<!-- Popular TV Shows row (Overseerr) -->
	{#if popularRow.items.length > 0}
		<MediaRow row={popularRow} />
	{/if}
</div>
