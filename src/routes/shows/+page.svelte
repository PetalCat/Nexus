...
<div class="flex flex-col gap-10 pb-10">
	...
	{#await data.trendingTV then trendingTV}
		...
		{#if h}
			...
			>
				{#if h.backdrop}
					<img src={h.backdrop} alt={h.title} class="absolute inset-0 h-full w-full object-cover" />
				{:else}
					<div class="absolute inset-0" style="background: linear-gradient(135deg, var(--color-raised) 0%, var(--color-deep) 60%, var(--color-base) 100%)">
				...
				{/if}
				<div class="absolute inset-0" style="background: linear-gradient(to top, var(--color-void) 0%, rgba(13,11,10,0.65) 35%, rgba(13,11,10,0.1) 70%, transparent 100%)"></div>
				...
				<div class="absolute bottom-0 left-0 right-0 flex items-end p-4 pb-5 sm:p-6 sm:pb-8 md:p-8 md:pb-10">
					<div class="max-w-xl">
						<div class="mb-3 flex flex-wrap items-center gap-2">
							...
							{#if h.duration}<span class="text-xs text-white/30">·</span><span class="text-xs font-medium text-white/50">{formatDuration(h.duration)}</span>{/if}
							{#if h.rating}
						...
						<div class="mt-4 flex items-center gap-3">
							<a href="/media/{h.type}/{h.sourceId}?service={h.serviceId}" class="flex items-center gap-2 rounded-xl border border-white/20 bg-white/10 px-5 py-2.5 text-sm font-semibold text-white backdrop-blur-sm transition-all hover:bg-white/20 active:scale-95">More Info</a>
						</div>
					</div>
				</div>
			</div>
