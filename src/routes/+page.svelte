<script lang="ts">
	...
	// Rows are pre-ordered by the server (see $lib/server/homepage-cache.ts CANONICAL
	// banner). The page renders them in the order received — no positional
	// hardcoding. Continue Watching and Calendar render via their own components
	// keyed on row.id; everything else goes through MediaRow.
	...
	{:else}
		<div class="mt-6 flex flex-col gap-10 pb-8">
			{#each data.rows as row (row.id)}
				...
				{:else if row.type === 'calendar' && row.calendarItems && row.calendarItems.length > 0}
					<CalendarRow items={row.calendarItems} title={row.title} subtitle={row.subtitle} />
				{:else}
					{@const dashRow = {
						id: row.id,
						title: row.title,
