...
<div class="px-3 py-4 sm:px-4 sm:py-6 lg:px-6">
	<div class="mb-6 max-w-xl">
		<SearchBar scope={data.typeFilter as any} />
	</div>
			...
			{#if requestableItems.length > 0}
				<section>
					<div class="mb-4">
						...
						<p class="mt-0.5 text-sm text-[var(--color-subtle)]">Found via Overseerr — request any of these to add them.</p>
					</div>
					<div class="flex flex-col gap-2">
						{#each requestableItems as item (item.id)}
							...
							<div class="card-raised flex items-center gap-4 px-4 py-3">
								...
								<div class="flex-1 min-w-0">
									...
									<div class="mt-1 flex items-center gap-2 flex-wrap">
										...
										{#if item.rating}
											<span class="text-[10px] text-[var(--color-star)] flex items-center gap-0.5">
										...
										{/if}
										{#if item.description}
											<span class="text-[10px] text-[var(--color-subtle)] line-clamp-1 flex-1 min-w-0 hidden sm:block">{item.description}</span>
										{/if}
									</div>
								</div>
								...
								<div class="flex-shrink-0">
									{#if reqState === 'done'}
										<span class="flex items-center gap-1 text-xs font-medium text-[var(--color-pulsar)]">
									...
									{:else if reqState === 'error'}
										<span class="text-xs text-[var(--color-nova)]">Failed</span>
									{:else}
										<button
											class="btn btn-ghost text-xs px-3 py-1.5 flex items-center gap-1.5 border border-[var(--color-nebula)]/30 text-[var(--color-nebula)] hover:bg-[var(--color-nebula)]/10"
