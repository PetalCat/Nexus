...
import { downloadRomState, deleteRomState } from '$lib/adapters/romm';
...
export const GET: RequestHandler = async ({ params, url, locals }) => {
	...
	const res = await downloadRomState(config, params.id, params.stateId, userCred);
	if (!res.ok) throw error(res.status, 'Failed to download state');
