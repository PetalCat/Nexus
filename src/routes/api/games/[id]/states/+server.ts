...
import { getRomStates, uploadRomState } from '$lib/adapters/romm';
...
export const GET: RequestHandler = async ({ params, locals, url }) => {
	...
	const states = await getRomStates(config, params.id, userCred);
...
export const POST: RequestHandler = async ({ params, request, locals, url }) => {
	...
	const ok = await uploadRomState(config, params.id, file, file.name, userCred, screenshot ?? undefined);
	if (!ok) {
