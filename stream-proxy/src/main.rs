...
async fn handle(
    ...
    if path == "/proxy" {
        let cdn_url = match query.split('&').find_map(|p| p.strip_prefix("url=")) {
            ...
            None => {
                return Ok(Response::builder().status(400).body(empty_body()).unwrap());
            }
        };
        ...
        let upstream = match upstream_req.send().await {
            ...
            Err(e) => {
                ...
                return Ok(Response::builder().status(502).body(empty_body()).unwrap());
            }
        };
        ...
        return Ok(builder.body(tracked_body.boxed()).unwrap());
    }
    ...
    Ok(builder.body(tracked_body.boxed()).unwrap())
}
