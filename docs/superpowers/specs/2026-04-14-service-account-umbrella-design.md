...
**Service**
A registered integration point — one row in the `services` table. Examples: *"the Jellyfin at `jellyfin.example.local:8096`"*, *"the Invidious at `invidious.example.com`"*. Services are global: there's one Jellyfin service shared by all Nexus users. The admin owns this row.
