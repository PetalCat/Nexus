...
This row represents "this Nexus install talks to a Jellyfin at `http://jellyfin.example.local:8096` using the admin API key `abc123`." It's shared across all Nexus users. When an admin sets up Nexus, they create these rows in the setup wizard or the services admin page. The `username` and `password` fields are **admin credentials** — Nexus uses them to call admin APIs like `getUsers()`, `resetPassword()`, `createUser()`. Not every service has them (Radarr uses `api_key` only; Invidious uses `url` only).
