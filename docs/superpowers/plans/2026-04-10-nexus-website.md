...
```bash
mkdir -p /path/to/dev/nexus-website
cd /path/to/dev/nexus-website
pnpm init
```
...
Create `static/favicon.svg` — reuse the Nexus star SVG from the main repo (`/path/to/dev/Nexus/static/favicon.svg`). Copy it over.
