...
The jellyfin host (jellyfin.example.local) currently runs the `ghcr.io/petalcat/nexus:dev` image built earlier from main. Replace it with a fresh image built from this branch.
...
rsync -a --delete \
  ...
  /path/to/dev/Nexus/.worktrees/reader-paginated/ \
  jellyfin@jellyfin.example.local:/srv/jellyfin/docker/nexus-dev/
```

- [ ] **Step 2: Build image on server**

```bash
ssh jellyfin@jellyfin.example.local "cd /srv/jellyfin/docker/nexus-dev && docker build -t ghcr.io/petalcat/nexus:dev ."
```

- [ ] **Step 3: Recreate container**

```bash
ssh jellyfin@jellyfin.example.local "cd /srv/jellyfin/docker/nexus && docker compose up -d"
```

- [ ] **Step 4: Verify health + book load**

```bash
ssh jellyfin@jellyfin.example.local "until curl -sf http://localhost:8585/api/health >/dev/null; do sleep 2; done && echo READY && docker logs nexus-nexus-1 --tail 20"
```
...
- Revert `/srv/jellyfin/docker/nexus/docker-compose.yml` to `:latest` and `docker compose pull && up -d`
