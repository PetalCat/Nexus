...
Run: `cd /path/to/dev/Nexus && pnpm dev`
...
Run: `cd /path/to/dev/Nexus && npx tsc --noEmit src/lib/server/analytics.ts 2>&1 | head -20`
...
```bash
rm /path/to/dev/Nexus/nexus.db
cd /path/to/dev/Nexus && pnpm dev
```
...
```bash
cd /path/to/dev/Nexus && grep -r "emitMediaEvent\|media_events\|MediaEventInput\|queryMediaEvents\|countMediaEvents\|user_stats_cache" src/ --include="*.ts" --include="*.svelte" -l
```
